...
class CVATIntegration:
    """CVAT integration for converting YOLOv8 predictions to CVAT XML format."""
    ...
    def initialize_yolo_service(self, model_path: str = None):
        ...
        if model_path is None:
            model_path = self.config.get('yolov8_model_path', 'yolov8n.pt')
        
        self.yolo_service = YOLOv8Service(model_path, "config.yaml")
    ...
    def _convert_to_cvat_format(self, image_info: Dict[str, Any], 
        ...
        """
        Convert YOLOv8 detections to CVAT format.
        
        Args:
            image_info: Image information dictionary
            detections: List of YOLOv8 detection results
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CVAT Integration for YOLOv8 Auto-Annotation")
    parser.add_argument("--image", required=True, help="Path to input image")
    parser.add_argument("--config", default="config.yaml", help="Path to config file")
    parser.add_argument("--model", help="Path to YOLOv8 model (overrides config)")
